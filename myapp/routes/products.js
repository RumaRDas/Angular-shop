...
router.get("/:prodId", (req, res) => {
  ...
  database
    ...
    .filter({ "p.id": productId }) //'p.id' that alies always use single cote
    .get()
    .then((prod) => {
      console.log(prod);
      if (prod) {
        ...
        res.json({ message: `No product found with the id ${productId}` });
      }
    })
    .catch((err) => res.json(err));
});

/*GET ALL PRODUCT FROM ONE CATEGORY */
  ...
  database
    ...
    .then((prods) => {
      if (prods.length > 0) {
        ...
        res.json({
          message: `No products found matching the category ${cat_title} category.`,
        });
      }
    })
    .catch((err) => res.json(err));
